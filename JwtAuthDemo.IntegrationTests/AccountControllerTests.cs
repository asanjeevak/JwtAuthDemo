...
namespace JwtAuthDemo.IntegrationTests
{
    ...
    public class AccountControllerTests
    {
        ...
        public async Task ShouldReturnCorrectResponseForSuccessLogin()
        {
            ...
            var loginResponse = await _httpClient.PostAsync("account/login",
                new StringContent(JsonSerializer.Serialize(credentials), Encoding.UTF8, MediaTypeNames.Application.Json));
        ...
        public async Task ShouldBeAbleToLogout()
        {
            ...
            var jwtAuthManager = _serviceProvider.GetRequiredService<IJwtAuthManager>();
            Assert.IsTrue(jwtAuthManager.UsersRefreshTokensReadOnlyDictionary.ContainsKey(credentials.UserName));
            ...
            var logoutResponse = await _httpClient.PostAsync("account/logout", null);
            Assert.AreEqual(HttpStatusCode.OK, logoutResponse.StatusCode);
            Assert.IsFalse(jwtAuthManager.UsersRefreshTokensReadOnlyDictionary.ContainsKey(credentials.UserName));
        }
        ...
        public async Task ShouldCorrectlyRefreshToken()
        {
            ...
            var response = await _httpClient.PostAsync("account/refresh-token",
            ...
            var refreshToken2 = jwtAuthManager.UsersRefreshTokensReadOnlyDictionary.GetValueOrDefault(userName);
        ...
        public async Task ShouldNotAllowToRefreshTokenWhenRefreshTokenIsExpired()
        {
            ...
            var response = await _httpClient.PostAsync("account/refresh-token",
                new StringContent(JsonSerializer.Serialize(jwtResult1.RefreshToken), Encoding.UTF8, MediaTypeNames.Application.Json)); // expired Refresh token
            var responseContent = await response.Content.ReadAsStringAsync();
            Assert.AreEqual(HttpStatusCode.Unauthorized, response.StatusCode);
            ...
            {
                UserName = jwtResult1.RefreshToken.UserName,
                TokenString = jwtResult1.RefreshToken.TokenString,
                ExpireAt = DateTime.Now.AddMinutes(20)
            }; // manipulated on the client side, but still is expired on the server side
            response = await _httpClient.PostAsync("account/refresh-token",
                new StringContent(JsonSerializer.Serialize(fakeRefreshToken), Encoding.UTF8, MediaTypeNames.Application.Json)); // fake Refresh token 
        ...
        public async Task ShouldAllowAdminImpersonateOthers()
        {
            ...
            var response = await _httpClient.PostAsync("account/impersonation",
        ...
        public async Task ShouldAllowAdminToStopImpersonation()
        {
            ...
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, jwtResult.AccessToken);
            var response = await _httpClient.PostAsync("account/stop-impersonation", null);
            var responseContent = await response.Content.ReadAsStringAsync();
            var result = JsonSerializer.Deserialize<LoginResult>(responseContent);
        ...
        public async Task ShouldReturnBadRequestIfStopImpersonationWhenNotImpersonating()
        {
            ...
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, jwtResult.AccessToken);
            var request = new ImpersonationRequest { UserName = "test2" };
            var response = await _httpClient.PostAsync("account/stop-impersonation",
                new StringContent(JsonSerializer.Serialize(request), Encoding.UTF8, MediaTypeNames.Application.Json));
